...
if type == 1
    str = ...
        sprintf('support vector number %d,account for %g%% (%d/%d)',model.totalSV,model.totalSV/size(data,1)*100,model.totalSV,size(data,1));
    disp(str);
    ratio = model.totalSV/size(data,1)*100;
    if ratio >= 80
        disp('Tips:support vector number is too large')
    end
else
    str = ...
        sprintf('support vector number %d',model.totalSV);
    disp(str);    
end
%% 各种分类准确率
disp('===accuracy===');
...
str = ...
    sprintf('total accuracy = %g%% (%d/%d)',CR.accuracy(1)*100,sum(plabel == label),numel(label));
disp(str)
for i = 1:numel(model.Label)
    ...
    str = ...
        sprintf('No %d class = %g%% (%d/%d)',model.Label(i),CR.accuracy(i+1)*100,p,p+n);
    disp(str);
end
%%
if type == 1
